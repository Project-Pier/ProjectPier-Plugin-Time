...
function download() {
  ...
  if ($output == 'pdf' ) {
    ...
    $subject = active_project()->getObjectName() .' '. lang(times);
    ...
    $pdf->Cell($w[0],6, $line);
    $pdf->Cell(60,6, lang('time kind'),1,0,'C',true);
    $pdf->Cell(40,6, lang('hour(s)') ,1,0,'C',true);
    $pdf->Cell(40,6, lang(user) ,1,0,'C',true);
    $pdf->Cell(0,6, lang(date) ,1,0,'C',true);
    $pdf->Ln();
    foreach($filtered as $time) {
      ...
      if ($tasklistname = ProjectTasklists::findById($time->getTasklistId())) {
        $tasklist_name = ProjectTasklists::findById($time->getTasklistId());
        $tasklist_name2 = $tasklist_name->getObjectName();

      } else {
        $tasklist_name2 = lang('no tasklistname added');
      }

      if ($taskname = ProjectTasks::findById($time->getTaskId())) {
        $task_name = ProjectTasks::findById($time->getTaskId());
        $task_name2 = $task_name->getObjectName();
      } else {
        $task_name2 = lang('no taskname added');
      }

      //$tasklist_name = ProjectTasklists::findById($time->getTasklistId());
      //$task_name = ProjectTasks::findById($time->getTaskId());
      $time_done = format_date($time->getDoneDate(), null, 0);
      if ($time->getIsBillable()) {
        $time_status = lang('billable time');
        $time_status_color_R = 0;
        $time_status_color_G = 150;
        $time_status_color_B = 0;
        $time_amount = $time->getHours();
        $time_amount_color_R = 0;
        $time_amount_color_G = 150;
        $time_amount_color_B = 0;
      } else {
        $time_status = lang('unbillable time');
        $time_status_color_R = 200;
        $time_status_color_G = 0;
        $time_status_color_B = 0;
        $time_amount = $time->getHours();
        $time_amount_color_R = 200;
        $time_amount_color_G = 0;
        $time_amount_color_B = 0;
      }
      if ($time->getAssignedTo()) {
        $time_assignee = $time->getAssignedTo()->getObjectName();
        $time_assignee_color_R = 0;
        $time_assignee_color_G = 0;
        $time_assignee_color_B = 0;
      } else {
        $time_assignee = lang('not assigned');
        $time_assignee_color_R = 200;
        $time_assignee_color_G = 0;
        $time_assignee_color_B = 0;
      }
      $pdf->Cell($w[0],6, $line);
      $pdf->SetFillColor(245,245,245);
      $pdf->SetTextColor($time_status_color_R, $time_status_color_G, $time_status_color_B);
      $pdf->Cell(60,6, $time_status,1,0,'C',true);
      $pdf->SetTextColor($time_amount_color_R, $time_amount_color_G, $time_amount_color_B);
      $pdf->Cell(40,6, $time_amount ." ". lang('hour(s)') ,1,0,'C',true);
      $pdf->SetTextColor($time_assignee_color_R, $time_assignee_color_G, $time_assignee_color_B);
      $pdf->Cell(40,6, $time_assignee ,1,0,'C',true);
      $pdf->SetTextColor(0, 0, 0);
      $pdf->Cell(0,6, $time_done ,1,0,'C',true);
      $pdf->Ln();
      $pdf->Cell($w[0],6,'');
      $pdf->MultiCell(0,6,$tasklist_name2 ." >> ". $task_name2 ." : \n \n". $time->getDescription(),1);
      //$pdf->Ln();
    }
    ...
    $s = "Project name\tId\tDate\tHours\tBillable\n";
    foreach($filtered as $time) {
      $s .= $project_name;
      $s .= "\t";
      $s .= $time->getId();
